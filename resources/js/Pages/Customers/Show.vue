...
<template>
    <Head :title="customer.name + ' — Requester'" />
                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg p-6">
                    <dl class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ customer.organization || '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Last Login</dt>
                            <dd class="mt-1 text-sm" :class="customer.last_login_at ? 'text-gray-900' : 'text-red-500'">
                                {{ timeAgo(customer.last_login_at) }}
                            </dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Account Created</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ new Date(customer.created_at).toLocaleDateString() }}</dd>
                        </div>
                    </dl>
                </div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div class="ml-4 flex items-center gap-2">
                                        <span v-if="ticket.duplicate_of" class="inline-flex rounded-full bg-purple-100 px-2 py-0.5 text-xs font-medium text-purple-700">duplicate</span>
